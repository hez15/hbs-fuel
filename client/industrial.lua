...
CreateThread(function()
    ...
    for _, refinery in pairs(Refineries) do
        ...
        if pts.crudeSource then
            helpZones[#helpZones + 1] = { coords = pts.crudeSource, label = 'Crude Oil Source — Grab hose to load crude tanker', colour = { 255, 120, 0 } }
        end
        for _, c in ipairs(pts.crudeDelivery or {}) do
            helpZones[#helpZones + 1] = { coords = c, label = 'Crude Delivery — Grab hose to unload crude into refinery', colour = { 255, 120, 0 } }
        end
        if pts.valve then
            helpZones[#helpZones + 1] = { coords = pts.valve, label = 'Refinery Valve — Open before starting a batch', colour = { 100, 180, 255 } }
        end
        if pts.processStart then
            helpZones[#helpZones + 1] = { coords = pts.processStart, label = 'Refinery Control — Start batch or view stock', colour = { 100, 180, 255 } }
        end
        if pts.tankerLoad then
            helpZones[#helpZones + 1] = { coords = pts.tankerLoad, label = 'Fuel Loading — Grab hose to load refined fuel into tanker', colour = { 50, 200, 100 } }
        end
        if pts.oilBottling then
            helpZones[#helpZones + 1] = { coords = pts.oilBottling, label = 'Oil Bottling — Package motor oil into bottles or drums', colour = { 200, 150, 50 } }
        end
    end

    for stationId, station in pairs(Stations) do
        for _, c in ipairs(station.unloadPoints or {}) do
            helpZones[#helpZones + 1] = { coords = c, label = 'Station Unload — Grab hose to deliver fuel from tanker', colour = { 50, 200, 100 } }
        end
    end

    for _, shop in pairs(Config.OilShops or {}) do
        for _, c in ipairs(shop.unloadPoints or {}) do
            helpZones[#helpZones + 1] = { coords = c, label = 'Oil Delivery — Grab hose to deliver motor oil', colour = { 200, 150, 50 } }
        end
    end
